...
export default function BFSSimulationPage(){
  ...
  <div className="container mx-auto px-4 py-12 max-w-screen-2xl">
      <Link href="/algorithms/graph/bfs" className="inline-flex items-center text-blue-600 hover:text-blue-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Back to BFS</Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-sm">
              ...
              <button onClick={resetGraph} className="px-4 py-2 rounded-lg bg-gray-200 text-gray-700 font-semibold hover:bg-gray-300 inline-flex items-center gap-1"><RotateCcw className="h-4 w-4"/>Reset</button>
...
function StatePanel({frame}:{frame?:Frame}){
  ...
  return <div className="grid md:grid-cols-3 gap-4 text-xs">
    <div className="p-3 rounded-xl bg-blue-50 border border-blue-200">
      ...
      <div className="flex flex-wrap gap-1 font-mono">{frame.queue.map(q=> <span key={q} className="px-2 py-1 rounded bg-white border border-blue-300">{q}</span>)}</div>
    </div>
    <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-300">
      <div className="font-semibold text-yellow-700 text-[11px] uppercase tracking-wide mb-1">Discovered</div>
      <div className="flex flex-wrap gap-1 font-mono">{frame.discovered.map(q=> <span key={q} className="px-2 py-1 rounded bg-white border border-yellow-400">{q}</span>)}</div>
    </div>
    <div className="p-3 rounded-xl bg-green-50 border border-green-200">
      <div className="font-semibold text-green-700 text-[11px] uppercase tracking-wide mb-1">Processed</div>
      <div className="flex flex-wrap gap-1 font-mono">{frame.processed.map(q=> <span key={q} className="px-2 py-1 rounded bg-white border border-green-300">{q}</span>)}</div>
    </div>
    <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 md:col-span-3">
...
function Legend(){
  return <div className="text-[11px] space-y-2">
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-blue-600 border-2 border-blue-700 inline-block"/> Current (front)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-yellow-400 border-2 border-yellow-500 inline-block"/> Discovered (queued)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-green-600 border-2 border-green-700 inline-block"/> Processed (done)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-white border-2 border-slate-300 inline-block"/> Undiscovered</div>
  </div>;
}
...
function Navigation(){
  return <div className="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div className="flex flex-wrap gap-3 justify-between items-center">
      <Link href="/algorithms/graph/bfs/theory" className="inline-flex items-center px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300"><ArrowLeft className="h-5 w-5 mr-2"/>Theory</Link>
      <Link href="/algorithms/graph/dfs" className="inline-flex items-center px-5 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Next: DFS <ArrowRight className="h-5 w-5 ml-2"/></Link>
