...
export default function DijkstraSimulationPage(){
  ...
  <div className="container mx-auto px-4 py-12 max-w-screen-2xl">
      <Link href="/algorithms/graph/dijkstra-shortest-path/theory" className="inline-flex items-center text-blue-600 hover:text-blue-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Back to Theory</Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-sm">
              <button onClick={addNode} className="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 inline-flex items-center gap-1"><Plus className="h-4 w-4"/>Add Node</button>
              ...
              <button onClick={randomizeGraph} className="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 inline-flex items-center gap-1"><Shuffle className="h-4 w-4"/>Randomize</button>
...
function GraphCanvas({nodes,setNodes,edges,setEdges,toggleEdge,cycleWeight,current}:{nodes:Node[];setNodes:React.Dispatch<React.SetStateAction<Node[]>>;edges:Edge[];setEdges:React.Dispatch<React.SetStateAction<Edge[]>>;toggleEdge:(a:string,b:string)=>void;cycleWeight:(id:string)=>void; current?:Frame}){
  ...
  return <div onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onDoubleClick={handleDoubleClick} className="relative w-full h-[380px] bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border border-slate-200 overflow-hidden select-none">
...
function Legend(){
  return <div className="text-[11px] space-y-2">
    ...
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-yellow-400 border-2 border-yellow-500 inline-block"/> Frontier (in PQ)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-green-600 border-2 border-green-700 inline-block"/> Settled (final)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-white border-2 border-slate-300 inline-block"/> Unreached</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-sm bg-emerald-500 inline-block" style={{boxShadow:'0 0 0 2px #05966955'}}/> Relaxation success edge</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-sm bg-blue-500 inline-block" style={{boxShadow:'0 0 0 2px #2563eb55'}}/> Relaxation check edge</div>
  </div>;
}
...
function Navigation(){
  return <div className="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div className="flex flex-wrap gap-3 justify-between items-center">
      <Link href="/algorithms/graph/dijkstra-shortest-path/theory" className="inline-flex items-center px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300"><ArrowLeft className="h-5 w-5 mr-2"/>Theory</Link>
      <Link href="/algorithms/graph" className="inline-flex items-center px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Graph Overview <ArrowRight className="h-5 w-5 ml-2"/></Link>
    </div>
  </div>;
}
