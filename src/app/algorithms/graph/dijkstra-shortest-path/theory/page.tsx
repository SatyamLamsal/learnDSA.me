...
function KeyConceptsCard(){
  return <Card title="Key Concepts" icon={<BrainCircuit className="h-5 w-5 text-indigo-600"/>}>
    <ul className="list-disc pl-5 text-sm text-slate-600 space-y-2">
      <li><span className="font-semibold text-gray-800">Relaxation:</span> Attempt to improve a tentative distance via an edge (u,v) by checking if dist[u] + w(u,v) &lt; dist[v].</li>
      <li><span className="font-semibold text-gray-800">Settled Set:</span> Vertices whose shortest distances are finalized; no future relaxation can improve them.</li>
      <li><span className="font-semibold text-gray-800">Priority Queue:</span> Orders frontier vertices by current tentative distance.</li>
      <li><span className="font-semibold text-gray-800">Stale Entries:</span> Older queue items with outdated distances; skipped when extracted.</li>
      <li><span className="font-semibold text-gray-800">Greedy Choice:</span> Always settle the closest unsettled vertex first—safe due to non‑negative weights.</li>
    </ul>
  </Card>;
}
    ...
    <div className="grid md:grid-cols-2 gap-5 text-sm text-slate-600">
      <div>
        ...
        <ul className="list-disc pl-5 space-y-1 text-gray-700">
          <li><span className="font-medium text-gray-600">dist[v]</span>: current best known distance</li>
          <li><span className="font-medium text-gray-600">parent[v]</span>: predecessor for path reconstruction</li>
          <li><span className="font-medium text-gray-600">visited[v]</span>: boolean settled flag</li>
        </ul>
      </div>
      <div>
...
function PitfallsCard(){
  return <Card title="Edge Cases & Pitfalls" icon={<AlertTriangle className="h-5 w-5 text-rose-600"/>}>
    <ul className="list-disc pl-5 text-sm text-slate-600 space-y-2">
      <li><span className="font-semibold text-gray-800">Negative Edge:</span> Invalidates correctness; use Bellman-Ford or Johnson.</li>
      <li><span className="font-semibold text-gray-800">Negative Cycle:</span> Not detectable by Dijkstra; distances undefined.</li>
      <li><span className="font-semibold text-gray-800">Unreachable Vertices:</span> Remain dist = ∞ (retain NIL parent).</li>
      <li><span className="font-semibold text-gray-800">Overflow:</span> Use large sentinel instead of Number.MAX_VALUE to avoid addition overflow in JS.</li>
      <li><span className="font-semibold text-gray-800">Dense Graph Choice:</span> O(V^2) array implementation might outperform heap.</li>
    </ul>
  </Card>;
}
...
function SideSummary(){
  return <div className="space-y-6 text-gray-700">
    ...
    <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
      ...
      <Link href="/algorithms/graph/dijkstra-shortest-path/simulation" className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold text-gray-300">Go to Simulation</Link>
    </motion.div>
  </div>;
}
...
function Navigation(){
  return <motion.div initial={{opacity:0,y:25}} animate={{opacity:1,y:0}} className="mt-14 flex justify-between items-center text-gray-700">
    <Link href="/algorithms/graph/dijkstra-shortest-path" className="inline-flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>Overview</Link>
    <div className="flex gap-4 text-gray-700">
      <Link href="/algorithms/graph/dijkstra-shortest-path/simulation" className="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-gray-100">Simulation <ArrowRight className="h-5 w-5 ml-2 text-gray-700"/></Link>
    </div>
  </motion.div>;
}
