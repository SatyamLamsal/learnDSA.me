...
export default function BellmanFordSimulationPage(){
  ...
  <div className="container mx-auto px-4 py-12 max-w-screen-2xl text-gray-700">
      <Link href="/algorithms/graph/bellman-ford/theory" className="inline-flex items-center text-rose-600 hover:text-rose-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>Back to Theory</Link>
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-gray-700">
        <div className="lg:col-span-2 space-y-6 text-gray-700">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-sm text-gray-600">
              <button onClick={addNode} className="px-4 py-2 rounded-lg bg-rose-600 text-white font-semibold hover:bg-rose-700 inline-flex items-center gap-1 text-gray-100"><Plus className="h-4 w-4 text-gray-700"/>Add Node</button>
              ...
              <button onClick={randomizeGraph} className="px-4 py-2 rounded-lg bg-amber-600 text-white font-semibold hover:bg-amber-700 inline-flex items-center gap-1 text-gray-100"><Shuffle className="h-4 w-4 text-gray-700"/>Randomize</button>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-6 text-gray-700">
          <div className="bg-white rounded-2xl shadow-sm p-6 border border-rose-200 text-gray-700">
...
function GraphCanvas({nodes,setNodes,edges,setEdges,toggleEdge,cycleWeight,current}:{nodes:Node[];setNodes:React.Dispatch<React.SetStateAction<Node[]>>;edges:Edge[];setEdges:React.Dispatch<React.SetStateAction<Edge[]>>;toggleEdge:(a:string,b:string)=>void;cycleWeight:(id:string)=>void; current?:Frame}){
  ...
  return <div onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onDoubleClick={handleDoubleClick} className="relative w-full h-[400px] bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border border-slate-200 overflow-hidden select-none text-gray-700">
    ...
    {nodes.map(n=> { const distVal = current? current.distances[n.id]: Infinity; const isSource = n.id=== 'N1'; return <button key={n.id} onMouseDown={(e)=> handleMouseDown(e,n.id)} onClick={()=> handleNodeClick(n.id)} style={{left:n.x-30, top:n.y-30}} className={`absolute h-16 w-16 rounded-full border-2 font-semibold flex flex-col items-center justify-center shadow bg-white text-slate-800 border-slate-300 cursor-move active:scale-95 text-[10px]`}><div>{n.id}</div><div className="font-mono text-gray-700">{distVal===Infinity?'∞':distVal}</div>{first===n.id && <span className="absolute -bottom-5 text-[10px] text-rose-600 font-mono">selecting</span>}</button>; })}
    {first && <div className="absolute top-2 left-2 text-[10px] px-2 py-1 bg-rose-600 text-white rounded text-gray-700">First: {first}</div>}
  </div>;
}
...
function StatePanel({frame,totalPasses}:{frame?:Frame; totalPasses:number}){
  ...
  return <div className="space-y-4 text-xs text-gray-600">
    <div className="grid md:grid-cols-3 gap-4 text-gray-700">
      <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-gray-700"><div className="font-semibold text-rose-700 text-[11px] uppercase tracking-wide mb-1">Pass</div><div className="font-mono text-gray-700">{frame.pass}/{totalPasses}</div></div>
      <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-gray-700"><div className="font-semibold text-amber-700 text-[11px] uppercase tracking-wide mb-1">Edge Index</div><div className="font-mono text-gray-700">{frame.edgeIndex < 0? '-' : frame.edgeIndex}</div></div>
      <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-gray-700"><div className="font-semibold text-emerald-700 text-[11px] uppercase tracking-wide mb-1">Updated This Step</div><div className="font-mono text-gray-700">{frame.relaxing? (frame.relaxing.improved? 'yes':'no'):'-'}</div></div>
    </div>
    ...
    {frame.negativeCycle && <div className="p-3 rounded-xl bg-rose-100 border border-rose-300 text-gray-700">
      <div className="font-semibold text-rose-700 text-[11px] uppercase tracking-wide mb-1 flex items-center gap-1"><AlertTriangle className="h-4 w-4 text-gray-700"/> Negative Cycle</div>
      <div className="text-[11px] text-rose-700">Further relaxations still possible; distances not well-defined.</div>
      {frame.cycleEdges && <div className="mt-2 flex flex-wrap gap-1 font-mono text-gray-700">{frame.cycleEdges.map((e,i)=> <span key={i} className="px-2 py-1 rounded bg-white border border-rose-400 text-gray-600">{e.from}→{e.to}</span>)}</div>}
    </div>}
  </div>;
}
...
function Legend(){
  return <div className="text-[11px] space-y-2 text-gray-700">
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-rose-600 inline-block text-gray-600"/> Relaxation improvement (edge)</div>
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-amber-500 inline-block text-gray-600"/> Relaxation check (no improve)</div>
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-red-600 inline-block text-gray-600"/> Negative cycle edge</div>
  </div>;
}
