...
export default function TopoSortTheory(){
  return <div className="min-h-screen bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 text-gray-100">
  <div className="container max-w-screen-2xl mx-auto px-4 py-12 text-gray-100">
      <Link href="/algorithms/graph/topological-sort" className="inline-flex items-center text-amber-600 hover:text-amber-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2 text-gray-100"/>Overview</Link>
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-gray-100">
        <div className="lg:col-span-2 space-y-8 text-gray-100">
          ...
          <Section title="Data Structures" icon={<Layers className="h-6 w-6 text-amber-600"/>}>
            <ul className="text-sm text-slate-600 space-y-2 leading-relaxed list-disc list-inside">
              <li><span className="font-semibold text-gray-100">Adjacency list:</span> Outgoing edges.</li>
              <li><span className="font-semibold text-gray-100">inDeg[v]:</span> Remaining prerequisites count.</li>
              <li><span className="font-semibold text-gray-100">Queue:</span> Ready vertices (in-degree 0).</li>
              <li><span className="font-semibold text-gray-100">Order array:</span> Progressive linearization.</li>
            </ul>
          </Section>
...
function Navigation(){
  return <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.55}} className="bg-white rounded-2xl shadow-sm border border-amber-100 p-6 text-gray-700">
    ...
    <div className="flex flex-col gap-3 text-gray-800">
      <Link href="/algorithms/graph/topological-sort/simulation" className="inline-flex items-center justify-center px-5 py-3 rounded-lg bg-amber-600 text-black font-semibold hover:bg-amber-700 text-gray-800">Simulation <ArrowRight className="h-5 w-5 ml-2 text-gray-800"/></Link>
