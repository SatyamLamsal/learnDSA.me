...
export default function TopoSortSimulationPage(){
  ...
  <div className="container mx-auto px-4 py-12 max-w-screen-2xl">
      <Link href="/algorithms/graph/topological-sort/theory" className="inline-flex items-center text-amber-600 hover:text-amber-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Back to Theory</Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-sm">
              <button onClick={addNode} className="px-4 py-2 rounded-lg bg-amber-600 text-white font-semibold hover:bg-amber-700 inline-flex items-center gap-1"><Plus className="h-4 w-4"/>Add Node</button>
              ...
              <button onClick={randomizeGraph} className="px-4 py-2 rounded-lg bg-orange-600 text-white font-semibold hover:bg-orange-700 inline-flex items-center gap-1"><Shuffle className="h-4 w-4"/>Randomize</button>
              <button onClick={introduceCycle} className="px-4 py-2 rounded-lg bg-red-600 text-white font-semibold hover:bg-red-700 inline-flex items-center gap-1"><RefreshCw className="h-4 w-4"/>Add Cycle</button>
...
function StatePanel({frame}:{frame?:Frame}){
  ...
  return <div className="space-y-4 text-xs">
    <div className="grid md:grid-cols-3 gap-4">
      <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-200"><div className="font-semibold text-yellow-700 text-[11px] uppercase tracking-wide mb-1">Queue</div><div className="flex flex-wrap gap-1 font-mono">{frame.queue.map(id=> <span key={id} className="px-2 py-1 rounded bg-white border border-yellow-400">{id}</span>)}</div></div>
      <div className="p-3 rounded-xl bg-green-50 border border-green-200"><div className="font-semibold text-green-700 text-[11px] uppercase tracking-wide mb-1">Order</div><div className="flex flex-wrap gap-1 font-mono">{frame.order.map(id=> <span key={id} className="px-2 py-1 rounded bg-white border border-green-300">{id}</span>)}</div></div>
      <div className="p-3 rounded-xl bg-amber-50 border border-amber-200"><div className="font-semibold text-amber-700 text-[11px] uppercase tracking-wide mb-1">Cycle</div><div className="font-mono">{frame.cycle? 'Yes':'No'}</div></div>
    </div>
    <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 overflow-x-auto"><div className="font-semibold text-purple-700 text-[11px] uppercase tracking-wide mb-1">In-Degrees</div><div className="flex flex-wrap gap-1 font-mono">{Object.entries(frame.inDegree).map(([k,v])=> <span key={k} className="px-2 py-1 rounded bg-white border border-purple-300">{k}:{v}</span>)}</div></div>
    {frame.removedEdge && <div className="p-3 rounded-xl bg-red-50 border border-red-200"><div className="font-semibold text-red-700 text-[11px] uppercase tracking-wide mb-1">Removed Edge</div><div className="text-[11px] font-mono">{frame.removedEdge.from}→{frame.removedEdge.to}</div></div>}
  </div>;
}

function Legend(){
  return <div className="text-[11px] space-y-2">
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-blue-600 border-2 border-blue-700 inline-block"/> Current processing</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-green-600 border-2 border-green-700 inline-block"/> Added to order</div>
    <div className="flex items-center gap-2"><span className="h-4 w-4 rounded-full bg-yellow-400 border-2 border-yellow-500 inline-block"/> In queue (ready)</div>
    <div className="flex items-center gap-2"><span className="h-4 w-6 rounded bg-red-600 inline-block"/> Edge removal</div>
  </div>;
}
...
function Navigation(){
  return <div className="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div className="flex flex-wrap gap-3 justify-between items-center">
      <Link href="/algorithms/graph/topological-sort/theory" className="inline-flex items-center px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300"><ArrowLeft className="h-5 w-5 mr-2"/>Theory</Link>
      <Link href="/algorithms/graph" className="inline-flex items-center px-5 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">Graph Overview <ArrowRight className="h-5 w-5 ml-2"/></Link>
    </div>
  </div>;
}
