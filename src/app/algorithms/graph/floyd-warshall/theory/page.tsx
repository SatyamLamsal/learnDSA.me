...
export default function FloydWarshallTheory(){
  ...
  <div className="container max-w-screen-2xl mx-auto px-4 py-12">
      <Link href="/algorithms/graph/floyd-warshall" className="inline-flex items-center text-indigo-600 hover:text-indigo-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Overview</Link>
...
function Section({title, icon, children}:{title:string; icon:React.ReactNode; children:React.ReactNode}){
  return <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6">
...
function Navigation(){
  return <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.5}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6">
    ...
    <div className="flex flex-col gap-3">
      <Link href="/algorithms/graph/floyd-warshall/simulation" className="inline-flex items-center justify-center px-5 py-3 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700">Simulation <ArrowRight className="h-5 w-5 ml-2"/></Link>
