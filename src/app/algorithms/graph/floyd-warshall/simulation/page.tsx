  ...
  <div className="container max-w-screen-2xl mx-auto px-4 py-12 text-gray-700">
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-gray-700">
        <div className="lg:col-span-2 space-y-8 text-gray-700">
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6 text-gray-700">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">Controls</h2>
            <div className="flex flex-wrap gap-3 items-center text-sm mb-4 text-gray-600">
              <button onClick={()=> setPlaying(p=> !p)} className="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 inline-flex items-center gap-1">{playing? <Pause className="h-4 w-4 text-gray-700"/> : <Play className="h-4 w-4 text-gray-700"/>}{playing? 'Pause':'Play'}</button>
              <button onClick={()=> setIdx(i=> Math.max(0,i-1))} disabled={idx===0} className="px-3 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 disabled:opacity-40"><StepBack className="h-4 w-4 text-gray-700"/></button>
              <button onClick={()=> setIdx(i=> Math.min(frames.length-1,i+1))} disabled={idx===frames.length-1} className="px-3 py-2 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 disabled:opacity-40"><StepForward className="h-4 w-4 text-gray-700"/></button>
              ...
              <button onClick={randomize} className="px-4 py-2 rounded-lg bg-rose-600 text-white font-semibold hover:bg-rose-700 inline-flex items-center gap-1 text-gray-100"><Shuffle className="h-4 w-4 text-gray-700"/>Random</button>
          ...
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6 text-gray-700">
            ...
            {current?.negativeCycle && <div className="mt-4 p-3 rounded-lg bg-rose-100 border border-rose-300 text-rose-700 text-sm inline-flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-gray-700"/> Negative Cycle Detected</div>}
          </motion.div>
        </div>
        <div className="space-y-8 text-gray-700">
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6 text-gray-700">
            ...
            <ul className="text-[11px] space-y-2 text-slate-600">
              <li><span className="inline-block w-4 h-4 rounded bg-indigo-200 border border-indigo-400 mr-2 text-gray-600"/> Current (i,j) under consideration</li>
              <li><span className="inline-block w-4 h-4 rounded bg-emerald-300 border border-emerald-500 mr-2 text-gray-600"/> Improvement applied</li>
              <li><span className="inline-block w-4 h-4 rounded bg-rose-300 border border-rose-500 mr-2 text-gray-600"/> Negative cycle (diagonal &lt; 0)</li>
            </ul>
          </motion.div>
          <Navigation />
        </div>
...
function MatrixView({frame}:{frame?:Frame}){
  ...
  return <div className="overflow-x-auto text-gray-700">
...
function Navigation(){
  return <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6 text-gray-700">
    <h2 className="text-lg font-semibold text-slate-800 mb-4">Continue</h2>
    <div className="flex flex-col gap-3 text-gray-700">
      <Link href="/algorithms/graph/floyd-warshall/theory" className="inline-flex items-center justify-center px-5 py-3 rounded-lg bg-gray-200 text-slate-700 font-semibold hover:bg-gray-300"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>Theory</Link>
      <Link href="/algorithms/graph" className="inline-flex items-center justify-center px-5 py-3 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 text-gray-100">All Graph Algorithms <ArrowRight className="h-5 w-5 ml-2 text-gray-700"/></Link>
    </div>
  </motion.div>;
}
