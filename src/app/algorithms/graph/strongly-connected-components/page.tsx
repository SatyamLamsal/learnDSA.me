  ...
  <div className="container max-w-screen-2xl mx-auto px-4 py-12">
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{duration:0.6}} className="mb-10">
        ...
        <h1 className="text-4xl font-bold text-slate-800 mb-4 flex items-center gap-3">
          <span className="inline-flex items-center justify-center h-14 w-14 rounded-2xl bg-fuchsia-600 text-white shadow">
      ...
      <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="flex justify-between items-center mt-16">
        <Link href="\algorithms\graph\topological-sort" className="inline-flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"><ArrowLeft className="h-5 w-5 mr-2"/>Back: topological-sort</Link>
      </motion.div>
    </div>
  </div>;
}

function Card({children}:{children:React.ReactNode}){ return <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.35}} className="bg-white rounded-2xl shadow-sm border border-fuchsia-100 p-6">{children}</motion.div>; }
...
function AlgorithmsCard(){
  return <Card>
    ...
    <ul className="text-sm text-slate-600 space-y-2 leading-relaxed list-disc list-inside">
      <li><span className="font-semibold">Kosaraju:</span> DFS order + transpose DFS.</li>
      <li><span className="font-semibold">Tarjan:</span> Single DFS with stack & lowlink.</li>
      <li><span className="font-semibold">Gabow:</span> Two stacks variant.</li>
    </ul>
  </Card>;
}
  ...
  return <Card>
    ...
    <div className="overflow-x-auto">
      <table className="text-xs w-full border border-slate-200">
        <thead className="bg-slate-100 text-slate-700"><tr><th className="p-2 text-left font-semibold">Algorithm</th><th className="p-2 text-left font-semibold">Passes</th><th className="p-2 text-left font-semibold">Key Structure</th><th className="p-2 text-left font-semibold">When Preferred</th></tr></thead>
        <tbody className="divide-y divide-slate-200">
          <tr className="hover:bg-fuchsia-50/40"><td className="p-2 font-mono">Kosaraju</td><td className="p-2">2 DFS + transpose</td><td className="p-2">Stack (finish order)</td><td className="p-2">Clarity, teaching</td></tr>
          <tr className="hover:bg-fuchsia-50/40"><td className="p-2 font-mono">Tarjan</td><td className="p-2">1 DFS</td><td className="p-2">Lowlink + stack</td><td className="p-2">Performance simplicity</td></tr>
          <tr className="hover:bg-fuchsia-50/40"><td className="p-2 font-mono">Gabow</td><td className="p-2">1 DFS</td><td className="p-2">Two stacks</td><td className="p-2">Alternate lowlink avoidance</td></tr>
        </tbody>
      </table>
    </div>
  </Card>;
...
function NavigationCard(){
  return <Card>
    ...
    <div className="flex flex-col gap-3">
      <Link href="/algorithms/graph/strongly-connected-components/theory" className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-fuchsia-600 text-white font-semibold hover:bg-fuchsia-700">Theory <ArrowRight className="h-5 w-5 ml-2"/></Link>
      <Link href="/algorithms/graph/strongly-connected-components/simulation" className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-slate-800 text-white font-semibold hover:bg-slate-900">Simulation <ArrowRight className="h-5 w-5 ml-2"/></Link>
