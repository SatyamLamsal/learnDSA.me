      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <Section title="Disjoint Set Union (DSU)" icon={<Layers className="h-6 w-6 text-indigo-600"/>}>
            ...
            <ul className="text-sm text-slate-600 space-y-2 leading-relaxed list-disc list-inside">
              <li><span className="font-semibold">make-set(x):</span> Initialize singleton.</li>
              <li><span className="font-semibold">find(x):</span> Return representative (with path compression).</li>
              <li><span className="font-semibold">union(a,b):</span> Merge roots (union by rank/size).</li>
          ...
          <Section title="Edge Cases" icon={<AlertTriangle className="h-6 w-6 text-indigo-600"/>}>
            <ul className="text-sm text-slate-600 space-y-2 leading-relaxed list-disc list-inside">
              <li><span className="font-semibold">Disconnected:</span> Produces minimum spanning forest (one per component).</li>
              <li><span className="font-semibold">Duplicate Weights:</span> Any order among equal weights still yields an MST.</li>
              <li><span className="font-semibold">Negative Weights:</span> Fully supported; sorting handles them.</li>
            </ul>
          </Section>
        </div>
...
function Section({title,icon,children}:{title:string;icon:React.ReactNode;children:React.ReactNode}){
  return <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.55}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6">
...
function Navigation(){
  return <motion.div initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.55}} className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-6">
    ...
    <div className="flex flex-col gap-3">
      <Link href="/algorithms/graph/kruskal/simulation" className="inline-flex items-center justify-center px-5 py-3 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700">Simulation <ArrowRight className="h-5 w-5 ml-2"/></Link>
