  ...
  <div className="container mx-auto px-4 py-12 max-w-screen-2xl text-gray-700">
      ...
      <div className="grid lg:grid-cols-3 gap-8 text-gray-700">
        <div className="lg:col-span-2 space-y-6 text-gray-700">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
            ...
            <div className="flex flex-wrap gap-3 mb-4 text-sm text-gray-600">
              <button onClick={addNode} className="px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 inline-flex items-center gap-1 text-gray-100">Add Node</button>
              ...
              <button onClick={randomizeGraph} className="px-4 py-2 rounded-lg bg-sky-600 text-white font-semibold hover:bg-sky-700 inline-flex items-center gap-1 text-gray-100">Randomize</button>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-6 text-gray-700">
          <div className="bg-white rounded-2xl shadow-sm p-6 border border-indigo-200 text-gray-700">
...
function StatePanel({frame}:{frame?:Frame}){
  ...
  return <div className="space-y-4 text-xs text-gray-600">
    ...
    <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 overflow-x-auto text-gray-700">
      <div className="font-semibold text-rose-700 text-[11px] uppercase tracking-wide mb-1">Ranks</div>
      <div className="flex flex-wrap gap-1 font-mono text-gray-700">{Object.entries(frame.ranks).map(([k,v])=> <span key={k} className="px-2 py-1 rounded bg-white border border-rose-300 text-gray-600">{k}:{v}</span>)}</div>
    </div>
    <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-gray-700">
      <div className="font-semibold text-slate-600 text-[11px] uppercase tracking-wide mb-1">Chosen Edges</div>
      <div className="flex flex-wrap gap-1 font-mono text-gray-700">{Array.from(frame.chosen).map(id=> <span key={id} className="px-2 py-1 rounded bg-white border border-emerald-300 text-gray-600">{id}</span>)}</div>
    </div>
  </div>;
}
...
function Legend(){
  return <div className="text-[11px] space-y-2 text-gray-700">
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-indigo-500 inline-block text-gray-600"/> Considering edge</div>
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-emerald-600 inline-block text-gray-600"/> Chosen (MST)</div>
    <div className="flex items-center gap-2 text-gray-700"><span className="h-3 w-6 rounded bg-amber-500 inline-block text-gray-600"/> Skipped (cycle)</div>
  </div>;
}
