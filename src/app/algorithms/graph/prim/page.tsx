...
export default function PrimOverview(){
  ...
  <div className="container max-w-screen-2xl mx-auto px-4 py-12">
      ...
      <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="flex justify-between items-center mt-16">
        <Link href="\algorithms\graph\kruskal" className="inline-flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"><ArrowLeft className="h-5 w-5 mr-2"/>Back: Kruskal</Link>
        <Link href="\algorithms\graph\topological-sort" className="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">Next:topological-sort <ArrowRight className="h-5 w-5 ml-2"/></Link>
      </motion.div>
    </div>
  </div>;
}

function Card({children}:{children:React.ReactNode}){ return <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:0.35}} className="bg-white rounded-2xl shadow-sm border border-emerald-100 p-6">{children}</motion.div>; }
  ...
  return <Card>
    ...
    <div className="overflow-x-auto">
      <table className="text-xs w-full border border-slate-200">
        <thead className="bg-slate-100 text-slate-700"><tr><th className="p-2 text-left font-semibold">Algorithm</th><th className="p-2 text-left font-semibold">Approach</th><th className="p-2 text-left font-semibold">Best For</th><th className="p-2 text-left font-semibold">Time</th></tr></thead>
        <tbody className="divide-y divide-slate-200">
          <tr className="hover:bg-emerald-50/40"><td className="p-2 font-mono">Prim (Heap)</td><td className="p-2">Grow tree</td><td className="p-2">Dense / general</td><td className="p-2">O(m log n)</td></tr>
          <tr className="hover:bg-emerald-50/40"><td className="p-2 font-mono">Prim (Matrix)</td><td className="p-2">Scan keys</td><td className="p-2">Very dense</td><td className="p-2">O(n^2)</td></tr>
          <tr className="hover:bg-emerald-50/40"><td className="p-2 font-mono">Kruskal</td><td className="p-2">Merge forests</td><td className="p-2">Sparse / sorted reuse</td><td className="p-2">O(m log n)</td></tr>
        </tbody>
      </table>
    </div>
  </Card>;
...
function NavigationCard(){
  return <Card>
    ...
    <div className="flex flex-col gap-3">
      <Link href="/algorithms/graph/prim/theory" className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-700">Theory <ArrowRight className="h-5 w-5 ml-2"/></Link>
      <Link href="/algorithms/graph/prim/simulation" className="inline-flex items-center justify-center px-6 py-3 rounded-lg bg-slate-800 text-white font-semibold hover:bg-slate-900">Simulation <ArrowRight className="h-5 w-5 ml-2"/></Link>
