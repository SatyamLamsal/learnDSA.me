  ...
  return <div className="min-h-screen bg-gradient-to-br from-sky-50 to-blue-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55}} className="mb-14">
        <Link href="/algorithms/divide-and-conquer/merge-sort" className="inline-flex items-center text-sky-600 hover:text-sky-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Overview</Link>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.05}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        ...
        <div className="flex flex-wrap gap-6 items-end">
          ...
          <button onClick={()=> setPlaying(p=> !p)} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-sky-600 text-white hover:bg-sky-700 text-xs font-semibold shadow">{playing? <Pause className="h-4 w-4 mr-2"/>: <Play className="h-4 w-4 mr-2"/>}{playing? 'Pause':'Play'}</button>
          ...
          <button onClick={()=> { const arr=Array.from({length:n},()=> Math.floor(Math.random()*99)+1); setData(arr); setIdx(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-xs font-semibold shadow"><RotateCcw className="h-4 w-4 mr-2"/>Reset</button>
          <button onClick={()=> { setData(d=> [...d]); setIdx(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 text-xs font-semibold shadow"><Shuffle className="h-4 w-4 mr-2"/>Rebuild</button>
        </div>
      </motion.div>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.1}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        ...
        {f.left && f.right && <div className="flex gap-6 mb-6">
          <div className="flex flex-col gap-1"><div className="text-[10px] uppercase font-semibold text-slate-500">Left</div><div className="flex flex-wrap gap-1">{f.left.map((v,i)=> <span key={i} className="px-2 py-1 rounded bg-slate-100 text-[11px] font-mono">{v}</span>)}</div></div>
          <div className="flex flex-col gap-1"><div className="text-[10px] uppercase font-semibold text-slate-500">Right</div><div className="flex flex-wrap gap-1">{f.right.map((v,i)=> <span key={i} className="px-2 py-1 rounded bg-slate-100 text-[11px] font-mono">{v}</span>)}</div></div>
        </div>}
        {f.merged && <div className="mb-4"><div className="text-[10px] uppercase font-semibold text-slate-500 mb-1">Merged</div><div className="flex flex-wrap gap-1">{f.merged.map((v,i)=> <span key={i} className="px-2 py-1 rounded bg-sky-100 text-[11px] font-mono">{v}</span>)}</div></div>}
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.15}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        <h2 className="text-2xl font-bold text-slate-800 mb-4 flex items-center gap-3"><Sigma className="h-6 w-6"/> Step Detail</h2>
        <div className="text-sm bg-gray-50 border border-gray-200 rounded p-5 leading-relaxed min-h-[110px] mb-8">{f.desc}</div>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Link href="/algorithms/divide-and-conquer/merge-sort/theory" className="inline-flex items-center px-6 py-3 rounded-md bg-sky-600 text-white hover:bg-sky-700 text-sm font-semibold">View Theory</Link>
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/simulation`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-sky-600 text-white hover:bg-sky-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
