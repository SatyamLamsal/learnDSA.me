...
export default function MergeSortTheory(){
  ...
  <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      {/* Recurrence & Work */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.05}} className="grid lg:grid-cols-2 gap-10 mb-16">
      ...
      {/* Notes & Properties */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.15}} className="bg-white rounded-2xl shadow-xl p-8 mb-16">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Link href="/algorithms/divide-and-conquer/merge-sort/simulation" className="inline-flex items-center px-6 py-3 rounded-md bg-sky-600 text-white hover:bg-sky-700 text-sm font-semibold">Run Simulation</Link>
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/theory`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-sky-600 text-white hover:bg-sky-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
