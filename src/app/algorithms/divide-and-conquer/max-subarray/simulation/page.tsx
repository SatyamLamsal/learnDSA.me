...
export default function MaxSubarraySimulation(){
  ...
  <div className="container mx-auto px-4 py-10 max-w-7xl">
      ...
      <div className="grid gap-6">
        ...
        {/* Controls */}
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.05,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 flex flex-wrap items-end gap-5">
          <div>
          ...
          <button onClick={()=> setPlaying(p=> !p)} className="inline-flex items-center px-4 py-2 rounded bg-amber-600 text-white hover:bg-amber-700 text-xs font-semibold">{playing? <Pause className="h-4 w-4 mr-2"/>: <Play className="h-4 w-4 mr-2"/>}{playing? 'Pause':'Play'}</button>
          ...
          <div className="text-[10px] font-mono">{index+1}/{frames.length}</div>
          ...
          <button onClick={reset} className="inline-flex items-center px-4 py-2 rounded bg-orange-600 text-white hover:bg-orange-700 text-xs font-semibold"><RotateCcw className="h-4 w-4 mr-2"/>Reset</button>
          <button onClick={()=> { setArray(a=> [...a]); setIndex(0); setPlaying(false); }} className="inline-flex items-center px-4 py-2 rounded bg-yellow-600 text-white hover:bg-yellow-700 text-xs font-semibold"><Shuffle className="h-4 w-4 mr-2"/>Rebuild</button>
        </motion.div>
        ...
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6">
          <div className="mb-3 flex items-center gap-2 text-xs font-semibold text-slate-600"><Split className="h-4 w-4"/> Segment [{f.lo}, {f.hi}] mid={f.mid}</div>
          <div className="flex gap-2 overflow-x-auto p-3 rounded-lg border bg-gradient-to-br from-white to-amber-50">
            {f.arr.map((v,i)=> {
          ...
          <div className="mt-3 grid md:grid-cols-3 gap-3 text-[10px]">
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.15,duration:0.5}} className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-lg font-semibold text-slate-800 mb-2">Step Detail</h2>
            <div className="text-sm bg-gray-50 border border-gray-200 rounded p-4 leading-relaxed min-h-[84px]">{explanation}</div>
        ...
        {/* Navigation */}
        <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:0.3,duration:0.5}} className="flex justify-between items-center">
          ...
          <Link href="/algorithms/divide-and-conquer/max-subarray/theory" className="px-6 py-2 rounded bg-amber-600 text-white hover:bg-amber-700">View Theory</Link>
...
function SuffixViz({frame}:{frame:Frame}){
  return <div className="mt-4">
    ...
    <div className="flex gap-1 flex-wrap text-[10px] font-mono">
      {(frame.suffixVals||[]).map((v,i)=><span key={i} className="px-2 py-1 rounded bg-amber-100 border border-amber-300">{v}</span>)}
    </div>
  </div>;
}
function PrefixViz({frame}:{frame:Frame}){
  return <div className="mt-4">
    ...
    <div className="flex gap-1 flex-wrap text-[10px] font-mono">
      {(frame.prefixVals||[]).map((v,i)=><span key={i} className="px-2 py-1 rounded bg-orange-100 border border-orange-300">{v}</span>)}
    </div>
  </div>;
}
