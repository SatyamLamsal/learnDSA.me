...
export default function MaxSubarrayOverview(){
  ...
  <div className="container mx-auto px-4 py-10 max-w-screen-2xl text-gray-700">
      ...
      <div className="grid gap-6 text-gray-700">
        ...
        {/* Insight Cards */}
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="grid md:grid-cols-3 gap-6 text-gray-700">
          <Insight icon={<Layers className="h-5 w-5 text-gray-700"/>} title="Structural Cases" color="bg-amber-600">Max subarray lies left, right, or crosses the midpoint.</Insight>
          <Insight icon={<Activity className="h-5 w-5 text-gray-700"/>} title="Cross Assembly" color="bg-orange-600">Crossing sum = best suffix left + best prefix right.</Insight>
          <Insight icon={<BarChart2 className="h-5 w-5 text-gray-700"/>} title="Contrast" color="bg-yellow-600">Kadane linear; D&C clarifies recurrence and merge-like scan.</Insight>
        </motion.div>
        ...
        {/* Actions */}
        <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 flex flex-wrap gap-4 text-gray-700">
          <Link href="/algorithms/divide-and-conquer/max-subarray/theory" className="px-6 py-2 rounded bg-amber-600 hover:bg-amber-700 text-white text-sm font-semibold text-gray-300">Deep Theory</Link>
        ...
        {/* Navigation */}
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.3,duration:0.5}} className="flex justify-between items-center text-gray-700">
...
function StatCard({label,value}:{label:string;value:string}){
  return <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100 text-gray-700">
