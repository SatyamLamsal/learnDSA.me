  ...
  return <div className="min-h-screen bg-gradient-to-br from-lime-50 to-green-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55}} className="mb-14">
        <Link href="/algorithms/divide-and-conquer/karatsuba" className="inline-flex items-center text-lime-600 hover:text-lime-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Overview</Link>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.05}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        ...
        <div className="flex flex-wrap gap-5 items-end">
          ...
          <button onClick={()=> setPlaying(p=> !p)} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-lime-600 text-white hover:bg-lime-700 text-xs font-semibold shadow">{playing? <Pause className="h-4 w-4 mr-2"/>: <Play className="h-4 w-4 mr-2"/>}{playing? 'Pause':'Play'}</button>
          <button onClick={()=> setIndex(i=> Math.max(0,i-1))} disabled={index===0} className="px-4 py-2.5 rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300 text-xs font-semibold disabled:opacity-40">Prev</button>
            <div className="text-[11px] font-mono tracking-tight">{index+1}/{frames.length}</div>
          ...
          <button onClick={()=> { setNums({x:randomBig(digits), y:randomBig(digits)}); setIndex(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-green-600 text-white hover:bg-green-700 text-xs font-semibold shadow"><RotateCcw className="h-4 w-4 mr-2"/>Reset</button>
          <button onClick={()=> { setNums(n=> ({...n})); setIndex(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 text-xs font-semibold shadow"><Shuffle className="h-4 w-4 mr-2"/>Rebuild</button>
        </div>
      </motion.div>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.1}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        <h2 className="text-2xl font-bold text-slate-800 mb-6 flex items-center gap-3"><Calculator className="h-6 w-6"/> Current Frame</h2>
        <div className="text-xs font-mono bg-slate-900 text-lime-100 rounded p-5 space-y-1">
          <div>x = {f.x}</div>
          <div>y = {f.y}</div>
          {f.x1 && <div className=""><span className="text-lime-300">x1</span> = {f.x1}  <span className="text-emerald-300">x0</span> = {f.x0}</div>}
          {f.y1 && <div className=""><span className="text-lime-300">y1</span> = {f.y1}  <span className="text-emerald-300">y0</span> = {f.y0}</div>}
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Link href="/algorithms/divide-and-conquer/karatsuba/theory" className="inline-flex items-center px-6 py-3 rounded-md bg-lime-600 text-white hover:bg-lime-700 text-sm font-semibold">View Theory</Link>
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/simulation`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-lime-600 text-white hover:bg-lime-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
