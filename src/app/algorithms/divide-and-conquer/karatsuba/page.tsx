  ...
  return <div className="min-h-screen bg-gradient-to-br from-lime-50 to-green-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55}} className="mb-14">
        <Link href="/algorithms/divide-and-conquer" className="inline-flex items-center text-lime-600 hover:text-lime-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>All Divide & Conquer</Link>
      ...
      {/* Mechanics */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.1}} className="grid lg:grid-cols-3 gap-10 mb-20">
        <Feature icon={<SplitSquareHorizontal className="h-6 w-6"/>} title="Digit Splitting" color="bg-lime-600">Split operands into high and low halves around midpoint digit length m.</Feature>
        <Feature icon={<Calculator className="h-6 w-6"/>} title="Three Products" color="bg-green-600">Compute z2 = x1*y1, z0 = x0*y0, and mid via (x1+x0)(y1+y0)-z2-z0.</Feature>
        <Feature icon={<Layers className="h-6 w-6"/>} title="Reconstruction" color="bg-emerald-600">Assemble: z2 * B^(2m) + z1 * B^m + z0 with base-power shifts.</Feature>
      </motion.div>
      ...
      {/* Performance Notes */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.2}} className="bg-white rounded-2xl shadow-xl p-8 mb-20">
        <h2 className="text-3xl font-bold text-slate-800 mb-6 flex items-center gap-3"><Info className="h-6 w-6"/> Performance Notes</h2>
        <ul className="list-disc pl-6 space-y-2 text-sm text-slate-700">
      ...
      {/* Actions */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.25}} className="flex flex-wrap gap-5 mb-14">
        <Link href="/algorithms/divide-and-conquer/karatsuba/theory" className="px-7 py-3.5 rounded-xl bg-lime-600 hover:bg-lime-700 text-white text-sm font-semibold shadow">Deep Theory</Link>
        <Link href="/algorithms/divide-and-conquer/karatsuba/simulation" className="px-7 py-3.5 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold shadow">Interactive Simulation</Link>
      </motion.div>
      ...
      <div className="flex justify-between items-center">
        ...
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-lime-600 text-white hover:bg-lime-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
...
function Feature({icon,title,children,color}:{icon:React.ReactNode; title:string; children:React.ReactNode; color:string;}){
  return <div className="bg-white/70 backdrop-blur border border-lime-200 rounded-xl p-4 flex flex-col shadow-sm">
