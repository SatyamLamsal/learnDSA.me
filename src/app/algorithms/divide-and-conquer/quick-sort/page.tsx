...
export default function QuickSortOverview(){
  ...
  <div className="container mx-auto px-4 py-10 max-w-7xl text-gray-700">
      <Link href="/algorithms/divide-and-conquer" className="inline-flex items-center text-amber-700 hover:text-amber-800 mb-8"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>All Divide & Conquer</Link>
      ...
      <div className="grid gap-6 text-gray-700">
        ...
        {/* Metrics */}
        <motion.div initial={{opacity:0,y:28}} animate={{opacity:1,y:0}} transition={{delay:0.05,duration:0.5}} className="grid sm:grid-cols-3 gap-4 text-gray-700">
        ...
        {/* Mechanics */}
        <motion.div initial={{opacity:0,y:26}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
        ...
        {/* Partition Behavior */}
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.15,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
          ...
          <div className="grid sm:grid-cols-3 gap-4 text-xs text-gray-600">
            <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 text-gray-700"><span className="font-semibold text-amber-700">Balanced split:</span> ~n/2 | ~n/2 → height ≈ log n</div>
            <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 text-gray-700"><span className="font-semibold text-amber-700">Skewed split:</span> 1 | n-1 → height n</div>
            <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 text-gray-700"><span className="font-semibold text-amber-700">Random pivot:</span> Skew probability decays exponentially</div>
          </div>
        </motion.div>

        {/* Recurrence & Guarantees */}
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.2,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm text-slate-600">
            <div>
              <p className="mb-2 text-gray-700"><span className="font-semibold text-slate-700">Average / Randomized:</span> T(n) ≈ 2T(n/2) + O(n) → O(n log n).</p>
              <p className="mb-2 text-gray-700"><span className="font-semibold text-slate-700">Worst:</span> T(n) = T(n-1) + O(n) → O(n^2) (pathological pivot chain).</p>
              <p className="mb-0 text-gray-700"><span className="font-semibold text-slate-700">Tail recursion:</span> last call often optimized / manually looped to reduce stack.</p>
            </div>
            <div className="space-y-2 text-xs leading-relaxed text-gray-600">
              <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 flex gap-2 text-gray-700"><Shield className="h-4 w-4 text-amber-600"/><span><b>Stability:</b> Not stable by default (partition swaps disrupt equal element order).</span></div>
              <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 flex gap-2 text-gray-700"><Gauge className="h-4 w-4 text-amber-600"/><span><b>Practical Speed:</b> Sequential linear scans & in-place operations yield excellent cache utilization.</span></div>
              <div className="p-3 rounded-lg bg-amber-50 border border-amber-100 flex gap-2 text-gray-700"><Zap className="h-4 w-4 text-amber-600"/><span><b>Heuristics:</b> Median-of-three, ninther, or random pivot reduce variance.</span></div>
            </div>
          </div>
        </motion.div>

        {/* Performance Notes */}
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
        ...
        {/* Navigation */}
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.35,duration:0.5}} className="flex justify-between items-center text-gray-700">
          ...
          {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}`} className="inline-flex items-center px-4 py-2 rounded bg-amber-600 text-white hover:bg-amber-700 text-gray-100">{next.name}<ArrowRight className="h-4 w-4 ml-2 text-gray-700"/></Link>: <span/>}
        </motion.div>
      </div>
    </div>
  </div>;
