...
export default function QuickSortSimulation(){
  ...
  return <div className="min-h-screen bg-gradient-to-br from-amber-50 to-yellow-50">
    <div className="container mx-auto px-4 py-10 max-w-7xl">
      <Link href="/algorithms/divide-and-conquer/quick-sort" className="inline-flex items-center text-amber-700 hover:text-amber-800 mb-8"><ArrowLeft className="h-5 w-5 mr-2"/>Overview</Link>
      <div className="grid gap-6">
        ...
        {/* Controls */}
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.05,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 flex flex-wrap gap-6 items-end">
          <div>
          ...
          <button onClick={()=> setPlaying(p=> !p)} className="inline-flex items-center px-5 py-2 rounded bg-amber-600 text-white hover:bg-amber-700 text-sm font-semibold shadow-sm">{playing? <Pause className="h-4 w-4 mr-2"/>: <Play className="h-4 w-4 mr-2"/>}{playing? 'Pause':'Play'}</button>
          <button onClick={()=> setIdx(i=> Math.max(0,i-1))} disabled={idx===0} className="px-4 py-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300 text-sm disabled:opacity-40">Prev</button>
            <div className="text-xs font-mono">{idx+1}/{frames.length}</div>
          ...
          <button onClick={()=> { const arr=Array.from({length:n},()=> Math.floor(Math.random()*99)+1); setData(arr); setIdx(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2 rounded bg-amber-600 text-white hover:bg-amber-700 text-sm font-semibold"><RotateCcw className="h-4 w-4 mr-2"/>Reset</button>
          <button onClick={()=> { setData(d=> [...d]); setIdx(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2 rounded bg-yellow-600 text-white hover:bg-yellow-700 text-sm font-semibold"><Shuffle className="h-4 w-4 mr-2"/>Rebuild</button>
        </motion.div>
        ...
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex flex-wrap items-center gap-4 mb-4 text-xs font-mono text-slate-600">
            <span className="px-2 py-1 bg-amber-600 text-white rounded">Pivot</span>
            <span className="px-2 py-1 bg-yellow-500 text-white rounded">Boundary i</span>
            ...
            <span className="px-2 py-1 bg-emerald-500 text-white rounded">Placed</span>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs text-slate-600 mt-2">
            <span className="font-semibold">Stage:</span><span>{f.stage}</span>
            {f.depth!==undefined && <><span className="font-semibold">Depth:</span><span>{f.depth}</span></>}
            {f.pivotVal!==undefined && <><span className="font-semibold">Pivot:</span><span>{f.pivotVal}</span></>}
          </div>
          {f.stage==='done' && <div className="text-sm font-semibold text-amber-700 mt-3">Array fully sorted</div>}
        </motion.div>

        {/* Step Detail & Pseudocode */}
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.15,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 grid lg:grid-cols-2 gap-8">
          <div>
            ...
            <div className="text-sm bg-gray-50 border border-gray-200 rounded p-4 leading-relaxed min-h-[120px] whitespace-pre-wrap">{f.desc}</div>
          </div>
          <div>
        ...
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.5}} className="flex justify-between items-center">
          ...
          <Link href="/algorithms/divide-and-conquer/quick-sort/theory" className="px-6 py-2 rounded bg-amber-600 text-white hover:bg-amber-700">View Theory</Link>
          {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/simulation`} className="inline-flex items-center px-4 py-2 rounded bg-amber-600 text-white hover:bg-amber-700">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
        </motion.div>
      </div>
    </div>
  </div>;
