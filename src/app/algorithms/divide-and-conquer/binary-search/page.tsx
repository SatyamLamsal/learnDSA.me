...
export default function BinarySearchOverview() {
  ...
  <div className="container mx-auto px-4 py-10 max-w-screen-2xl">
      ...
      <div className="grid gap-6">
        ...
        {/* Mechanics */}
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6">
        ...
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.15,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm text-slate-600">
            ...
            <div className="grid gap-3 text-xs">
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2"><Gauge className="h-4 w-4 text-emerald-600"/><span><b>Cost per step:</b> Single comparison + pointer move.</span></div>
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2"><Shield className="h-4 w-4 text-emerald-600"/><span><b>Overflow safe:</b> mid = lo + ((hi - lo) {'>>'} 1).</span></div>
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2"><BinaryIcon className="h-4 w-4 text-emerald-600"/><span><b>Variants:</b> lower/upper bound, first true, binary on answer.</span></div>
            </div>
          </div>
        </motion.div>
        ...
        {/* Performance Notes */}
        <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6">
        ...
        {/* Actions */}
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.3,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 flex flex-wrap gap-4">
          <Link href="/algorithms/divide-and-conquer/binary-search/theory" className="inline-flex items-center px-6 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold"><BookOpen className="h-5 w-5 mr-2"/>Theory</Link>
          <Link href="/algorithms/divide-and-conquer/binary-search/simulation" className="inline-flex items-center px-6 py-2 rounded bg-slate-200 text-slate-700 hover:bg-slate-300 text-sm font-semibold"><Play className="h-5 w-5 mr-2"/>Simulation</Link>
        </motion.div>

        {/* Navigation */}
        <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:0.35,duration:0.5}} className="flex justify-between items-center">
          ...
          {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}`} className="inline-flex items-center px-4 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
        </motion.div>
      </div>
    </div>
  </div>;
...
function StatCard({label,value}:{label:string;value:string}){
  return <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
