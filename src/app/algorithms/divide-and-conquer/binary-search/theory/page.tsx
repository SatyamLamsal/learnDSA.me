...
export default function BinarySearchTheory(){
  ...
  <div className="container mx-auto px-4 py-10 max-w-screen-2xl text-gray-700">
      ...
      <div className="grid gap-6 text-gray-700">
        ...
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm text-slate-600">
            ...
            <div className="grid gap-3 text-xs text-gray-600">
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2 text-gray-700"><Gauge className="h-4 w-4 text-emerald-600"/><span><b>Comparisons:</b> ⌊log2 n⌋ + O(1) worst.</span></div>
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2 text-gray-700"><Shield className="h-4 w-4 text-emerald-600"/><span><b>Overflow safe mid:</b> lo + ((hi - lo) {'>>'} 1).</span></div>
              <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-100 flex gap-2 text-gray-700"><Zap className="h-4 w-4 text-emerald-600"/><span><b>Answer-space use:</b> Search monotonic predicates (e.g., feasibility tests).</span></div>
            </div>
          </div>
        </motion.div>
        ...
        {/* Applications */}
        <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:0.3,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 text-gray-700">
        ...
        {/* Navigation */}
        <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:0.35,duration:0.5}} className="flex justify-between items-center text-gray-700">
          ...
          <Link href="/algorithms/divide-and-conquer/binary-search/simulation" className="px-6 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold text-gray-300">Go to Simulation</Link>
          {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/theory`} className="inline-flex items-center px-4 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700 text-gray-100">{next.name}<ArrowRight className="h-4 w-4 ml-2 text-gray-700"/></Link>: <span/>}
        </motion.div>
      </div>
    </div>
  </div>;
