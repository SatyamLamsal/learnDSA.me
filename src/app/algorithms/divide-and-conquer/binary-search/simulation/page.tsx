    ...
    <div className="container mx-auto px-4 py-10 max-w-7xl">
      ...
      <div className="grid gap-6">
        ...
        <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:0.05,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 flex flex-wrap gap-6 items-end">
          ...
          <button onClick={()=> setPlaying(p=> !p)} className="inline-flex items-center px-5 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold shadow-sm">{playing? <Pause className="h-4 w-4 mr-2"/>: <Play className="h-4 w-4 mr-2"/>}{playing? 'Pause':'Play'}</button>
          ...
          <div className="text-xs font-mono">{idx+1}/{steps.length}</div>
          ...
          <button onClick={reset} className="inline-flex items-center px-5 py-2 rounded bg-purple-600 text-white hover:bg-purple-700 text-sm font-semibold"><RotateCcw className="h-4 w-4 mr-2"/>Reset Data</button>
          <button onClick={()=> { const sorted=[...base]; const t=sorted[Math.floor(Math.random()*sorted.length)]; setDataset({base:sorted,target:t}); setIdx(0); setPlaying(false); }} className="inline-flex items-center px-5 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700 text-sm font-semibold"><Shuffle className="h-4 w-4 mr-2"/>New Target</button>
        </motion.div>

        {/* Visualization */}
        <motion.div initial={{opacity:0,y:22}} animate={{opacity:1,y:0}} transition={{delay:0.1,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 overflow-x-auto">
          <div className="flex flex-wrap items-center gap-3 mb-4 text-xs font-mono text-slate-600">
            <span className="px-2 py-1 bg-emerald-600 text-white rounded">Found</span>
            <span className="px-2 py-1 bg-pink-500 text-white rounded">mid</span>
          ...
          <div className="flex gap-2 items-end">
            {s.arr.map((v,i)=> {
              ...
              return <div key={i} className="flex flex-col items-center" style={{minWidth:'44px'}}>
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.15,duration:0.5}} className="bg-white rounded-2xl shadow-sm p-6 grid lg:grid-cols-2 gap-8">
          <div>
            ...
            <div className="text-sm bg-gray-50 border border-gray-200 rounded p-4 leading-relaxed min-h-[110px]">
        ...
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.25,duration:0.5}} className="flex justify-between items-center">
          ...
          <Link href="/algorithms/divide-and-conquer/binary-search/theory" className="px-6 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700">View Theory</Link>
          {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/simulation`} className="inline-flex items-center px-4 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
        </motion.div>
      </div>
    </div>
  </div>;
