...
export default function FFTTheory(){
  return <div className="min-h-screen bg-gradient-to-br from-fuchsia-50 to-pink-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55}} className="mb-14">
        <Link href="/algorithms/divide-and-conquer/fft" className="inline-flex items-center text-fuchsia-600 hover:text-fuchsia-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Overview</Link>
  ...
  <p className="text-base text-slate-700">Compute length-n DFT faster than naive O(n^2) summation: X[k] = sum a[j] * exp(-2*pi*i*j*k / n). Use parity split to form two size n/2 transforms.</p>
      ...
      {/* Edge + Complexity */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.3}} className="grid lg:grid-cols-2 gap-10 mb-14">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Link href="/algorithms/divide-and-conquer/fft/simulation" className="inline-flex items-center px-6 py-3 rounded-md bg-fuchsia-600 text-white hover:bg-fuchsia-700 text-sm font-semibold">Simulation</Link>
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/theory`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-fuchsia-600 text-white hover:bg-fuchsia-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
