  ...
  return <div className="min-h-screen bg-gradient-to-br from-fuchsia-50 to-pink-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55}} className="mb-12">
        <Link href="/algorithms/divide-and-conquer" className="inline-flex items-center text-fuchsia-600 hover:text-fuchsia-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>All Divide & Conquer</Link>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.1}} className="bg-white rounded-2xl shadow-xl p-8 mb-14">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <Feature icon={<Waves className="h-5 w-5"/>} title="Frequency Domain" color="bg-fuchsia-600">Transforms time-domain samples into frequency spectrum.</Feature>
          <Feature icon={<Signal className="h-5 w-5"/>} title="Convolution Power" color="bg-pink-600">Fast polynomial & signal convolution via pointwise multiply.</Feature>
          <Feature icon={<Activity className="h-5 w-5"/>} title="Butterflies" color="bg-rose-600">Pair operations reuse partial sums / differences.</Feature>
        </div>
      </motion.div>
      ...
      {/* Actions */}
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.2}} className="flex flex-wrap gap-5 mb-10">
        <Link href="/algorithms/divide-and-conquer/fft/theory" className="inline-flex items-center px-7 py-3.5 bg-fuchsia-600 text-white rounded-lg hover:bg-fuchsia-700 text-sm font-semibold shadow">Deep Theory</Link>
        <Link href="/algorithms/divide-and-conquer/fft/simulation" className="inline-flex items-center px-7 py-3.5 bg-pink-600 text-white rounded-lg hover:bg-pink-700 text-sm font-semibold shadow">Interactive Simulation</Link>
      </motion.div>
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-fuchsia-600 text-white hover:bg-fuchsia-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
...
function Feature({icon,title,children,color}:{icon:React.ReactNode; title:string; children:React.ReactNode; color:string;}){
  return <div className="bg-white/70 backdrop-blur border border-fuchsia-200 rounded-xl p-4 flex flex-col shadow-sm">
