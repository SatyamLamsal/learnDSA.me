  ...
  return <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-violet-50">
    <div className="container mx-auto px-4 py-16 max-w-7xl">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.1}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        ...
        <div className="text-[11px] font-mono bg-indigo-50 border border-indigo-200 rounded p-4 space-y-1 mb-6">
        ...
        <div className="text-[11px] font-mono bg-violet-50 border border-violet-200 rounded p-4 space-y-1">
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.2}} className="bg-white rounded-2xl shadow-xl p-8 mb-12">
        <h2 className="text-3xl font-bold text-slate-800 mb-6 flex items-center gap-3"><Lightbulb className="h-6 w-6"/> Key Insights</h2>
      ...
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.55, delay:0.25}} className="grid lg:grid-cols-2 gap-10 mb-14">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <h2 className="text-2xl font-bold text-slate-800 mb-5 flex items-center gap-2"><Info className="h-5 w-5"/> Edge Considerations</h2>
          <ul className="list-disc pl-6 space-y-2 text-sm text-slate-700">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <Link href="/algorithms/divide-and-conquer/strassen/simulation" className="inline-flex items-center px-6 py-3 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 text-sm font-semibold">Simulation</Link>
        {next? <Link href={`/algorithms/divide-and-conquer/${next.slug}/theory`} className="inline-flex items-center px-5 py-2.5 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 text-xs font-semibold">{next.name}<ArrowRight className="h-4 w-4 ml-2"/></Link>: <span/>}
      </div>
    </div>
  </div>;
}
