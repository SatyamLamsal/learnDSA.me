...
export default function CoinChangeGreedyPage(){
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-amber-50 text-gray-700">
      <div className="container mx-auto px-4 py-12 text-gray-700">
        <Link href="/algorithms/greedy" className="inline-flex items-center text-rose-600 hover:text-rose-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>Back to Greedy</Link>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8 text-gray-700">
          <div className="flex flex-wrap items-end gap-3 mb-4 text-gray-700">
            ...
            <button onClick={run} disabled={running} className="bg-rose-600 hover:bg-rose-700 text-black px-4 py-2 rounded inline-flex items-center disabled:opacity-50 text-black text-black text-black text-black"><Play className="h-4 w-4 mr-2 text-gray-700"/>Run</button>
            ...
            <button onClick={randomize} className="px-3 py-2 border rounded text-gray-800">Randomize Denoms</button>
          </div>
          <div className="flex gap-3 flex-wrap items-start text-gray-700">
              ...
              return (
                <motion.div key={d} initial={{opacity:0, y:6}} animate={{opacity:1, y:0}} className={`p-4 border rounded w-44 ${isActive? 'bg-rose-50 border-rose-300' : 'bg-gray-50'}`}>
                  <div className="flex items-center justify-between text-gray-700">
                    ...
                    <button className="text-xs underline text-gray-600" onClick={()=>{
                  ...
                  <div className="mt-2 h-2 bg-gray-200 rounded overflow-hidden text-gray-700">
                    <motion.div className="h-2 bg-rose-500 text-gray-700" initial={{width:'0%'}} animate={{width: `${Math.min(100, cnt*20)}%`}}/>
                  </div>
                </motion.div>
              );
            })}
          </div>
          <div className="mt-4 p-3 border rounded bg-white/70 text-gray-700">
            <div className="text-sm text-slate-700">Remaining amount: <span className="font-semibold text-gray-800">{remaining}</span></div>
            <div className="mt-2 h-3 bg-gray-100 rounded overflow-hidden text-gray-700">
              <motion.div className="h-3 bg-emerald-500 text-gray-700" initial={{width:'0%'}} animate={{width: `${amount===0?0:Math.max(0, Math.min(100, ((amount-remaining)/amount)*100))}%`}}/>
            </div>
