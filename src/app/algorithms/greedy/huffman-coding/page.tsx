...
export default function HuffmanCodingPage(){
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-fuchsia-50 text-gray-700">
      <div className="container mx-auto px-4 py-12 text-gray-700">
        <Link href="/algorithms/greedy" className="inline-flex items-center text-purple-600 hover:text-purple-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2 text-gray-700"/>Back to Greedy</Link>
              ...
              <div className="space-y-2 mb-4 text-gray-700">
                {symbols.map((s, i)=> (
                  <div key={i} className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <button className="text-xs px-2 py-1 border rounded text-gray-600" onClick={()=> setSymbols(prev=> prev.filter((_,idx)=> idx!==i))}>Remove</button>
                  </div>
                ))}
                <button className="text-xs px-2 py-1 border rounded text-gray-600" onClick={()=> setSymbols(prev=> [...prev, { id: String.fromCharCode(65+(prev.length%26)), freq: Math.floor(Math.random()*40)+5 }])}>Add Symbol</button>
              </div>
            </div>
            <div>
              <h3 className="font-semibold text-slate-700 mb-2">Min-Heap Levels</h3>
              <div className="space-y-3 min-h-[4rem] text-gray-700">
                {steps.length===0 && (
