    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-emerald-50">
      <div className="container mx-auto px-4 py-12">
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.8}} className="mb-8">
          <Link href="/algorithms/searching" className="inline-flex items-center text-teal-600 hover:text-teal-700 mb-6"><ArrowLeft className="h-5 w-5 mr-2"/>Back to Searching Algorithms</Link>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <button onClick={onInsert} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded">Insert</button>
            <button onClick={onSearch} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">Search</button>
            <button onClick={onRemove} className="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded">Remove</button>
        ...
        <div className="bg-white rounded-lg shadow p-6">
                ...
                <div key={idx} className={`border-2 rounded p-3 ${highlight ? 'border-emerald-600 bg-emerald-50' : 'border-gray-200'}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {bucket.length === 0 ? (
                      ...
                      <span key={i} className="px-2 py-1 text-xs bg-gray-100 rounded border border-gray-200">{k}</span>
                    ))}
                  </div>
                </div>
              );
          ...
          {lastOp && lastOp.type==='search' && (
            <div className={`mt-4 inline-flex items-center px-4 py-2 rounded border ${lastOp.found ? 'bg-green-50 text-green-800 border-green-300' : 'bg-red-50 text-red-800 border-red-300'}`}>
              {lastOp.found ? <CheckCircle2 className="h-5 w-5 mr-2"/> : <XCircle className="h-5 w-5 mr-2"/>}
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.8, delay:0.2}} className="flex justify-between items-center mt-10">
          <Link href="/algorithms/searching/fibonacci-search" className="inline-flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors"><ArrowLeft className="h-5 w-5 mr-2"/>Previous: Fibonacci Search</Link>
          <Link href="/algorithms/searching" className="inline-flex items-center px-6 py-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors">Back to Searching Index<ArrowRight className="h-5 w-5 ml-2"/></Link>
        </motion.div>
      </div>
    </div>
  );
